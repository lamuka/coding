...
class TelegramBot
{
	...
	[	
		...
		'default_admin1' => ['keyboard' => [['Создать', 'Запустить'], ['Инфо', 'Изменить'], ['Статистика', 'Рассылки'], ['Архив', 'Баны'], ['Розыгрыши без билетов']], 'resize_keyboard' => true],
		
		'default_admin' => ['keyboard' => [['Создать', 'Запустить'], ['Инфо', 'Изменить'], ['Статистика', 'Рассылки'], ['Архив', 'Баны']], 'resize_keyboard' => true],
...
/////	
	...
	public $testusers='000000000, 111111111111, 222222222222, 3333333333333';
/////	
...
/////
	...
	{	
		...
		if($this->cmd_if_exists($table))
		{			
			if(empty(mysqli_num_rows(mysqli_query($con_sql2, 'SELECT 1 FROM '.$table.' LIMIT 1'))))
			{
				return true;
			}
		}
...
/////	
	public function if_maingroup_short($lotteryname)
	{
		...
		return false;
												...
												////////////USERS SHORT////////////
/////
	public function cmd_start_shortlottery($lotteryname, $telegramchatid)
	{
		$con_sql2=$this->cmd_sql();
		...
		if($this->cmd_if_exists($shortlottery))
		{
			...
			if($this->if_mainchannel_short($lotteryname))
			{
				...
				elseif($this->inchannel_check_short($lotteryname, $chatid)==FALSE)
				{
				...
				elseif($this->cmd_sql_searchchatidtable($chatid, $tabcasinousers)==FALSE)
				{
					...
					$text='Чтобы принимать участие в розыгрышах, Вам необходимо <b>иметь аккаунт в Казино</b>.'.PHP_EOL.''.PHP_EOL.'Если у Вас еще <b>нет аккаунта</b>, перейдите на сайт Казино нажав кнопку <b>🎰Перейти на сайт</b>.'.PHP_EOL.''.PHP_EOL.'Если Вы <b>уже зарегистрированы</b>, пришлите мне Ваш ID (номер счета) в Казино👇';
				
					mysqli_query($con_sql2, 'CREATE TABLE IF NOT EXISTS '.$shortuserservice.' (id INT(5) UNSIGNED AUTO_INCREMENT PRIMARY KEY,info VARCHAR(15) DEFAULT "0") ENGINE = InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;');
					usleep(250000);
					
					mysqli_query($con_sql2, 'TRUNCATE TABLE '.$shortuserservice.'');
					usleep(250000);
					
					mysqli_query($con_sql2, 'INSERT INTO '.$shortuserservice.' (info) VALUES ("'.$lotteryname.'")');
					usleep(250000);
				...
				{
					if($this->cmd_sql_searchchatidtable($chatid, $shortuserslottery))
					{
			
						$text='<b>Вы уже зарегистрированы</b> в Розыгрыше №'.$lotteryname.' от '.$startdate.'!';
			...
			else
			{
				if($this->testusers($chatid))
				{
					...
					elseif($this->cmd_sql_searchchatidtable($chatid, $tabcasinousers))
					{
						if($this->cmd_sql_searchchatidtable($chatid, $shortuserslottery))
						{
							...
							$text='Вы уже <b>зарегистрированы</b> в Розыгрыше №'.$lotteryname.' от '.$startdate.'!';
				...
				{
					$text='<b>К сожалению, данная команда недопустима!</b>';
...
/////
	public function callback_refresh_short_status($lotteryname)
	{
		...
		$this->cmd_start_shortlottery($lotteryname, $chatid);
...
/////
	public function cmd_id_casino_short_check($lotteryname, $chatid)
	{
		$con_sql2=$this->cmd_sql();
		...
		if($this->cmd_if_exists($shortlottery))
		{
			...
			$startdate = date('d-m-Y', strtotime('-7 days', strtotime($row[0][0])));
			
			if(preg_match('/^\d{9,10}$/', $textmessage) && $textmessage!=$chatid && $this->cmd_sql_searchcasinoidtable($textmessage, $tabcasinousers)==FALSE)
			{
				if($this->cmd_if_exists($tabtempuser)==FALSE)
				{
					mysqli_query($con_sql2, 'CREATE TABLE IF NOT EXISTS '.$tabtempuser.' (id INT(5) UNSIGNED AUTO_INCREMENT PRIMARY KEY,info VARCHAR(15) DEFAULT "0") ENGINE = InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;');
					usleep(250000);
					...
					$postdata=http_build_query(
						[
							...
							'idtelegram' => ''.$chatid.'shortlottery'.$lotteryname.'',
							'startdate' => $startdate
						]
					);
					...
					$text='<b>Ваш запрос с ID '.$textmessage.' успешно отправлен</b>😇'.PHP_EOL.''.PHP_EOL.'Вам вскоре придет уведомление.';
					...
					@file_put_contents(''.dirname(__FILE__).'/id1.txt', 'registration short date - '.date('d.m.Y G:i:s').''.PHP_EOL.'idcasino - '.$textmessage.', idtelegram - '.$chatid.'shortlottery'.$lotteryname.''.PHP_EOL.''.PHP_EOL.'',FILE_APPEND);
				}
				else
				{
					$text='<b>Ваш предыдущий запрос ещё в обработке</b>😇'.PHP_EOL.''.PHP_EOL.'Подождите немного. Вам вскоре придет уведомление.';
			...
			else
			{
				...
				if($counter!=$counterlimit)
				{
					...
					$text='<b>Я не смог найти Ваш ID</b> ('.$textmessage.'), потому что его нет в Казино😔'.PHP_EOL.''.PHP_EOL.'Скорее всего, он был указан неправильно.'.PHP_EOL.''.PHP_EOL.'Пожалуйста, пришлите еще раз Ваш номер счета, чтобы Вы могли участвовать в розыгрышах👇'.PHP_EOL.''.PHP_EOL.'<i>Осталось попыток - '.$tries.'</i>';
				...
				else
				{
					...
					mysqli_query($con_sql2, 'DROP TABLE IF EXISTS '.$shortuserservice.'');
					usleep(250000);
					...
					mysqli_query($con_sql2, 'DELETE FROM '.$shortuserslottery.' WHERE chatid="'.$chatid.'"');
					usleep(250000);
					...
					$text='<b>Вы 5 раз прислали мне непонятно что</b>🥸'.PHP_EOL.''.PHP_EOL.'К сожалению, вынужден Вас заблокировать.'.PHP_EOL.''.PHP_EOL.'Чтобы участвовать в розыгрышах, напишите моему старшему - @'.$usernameadmin.'.';
...
/////
public function cmd_short_data_casino($error, $chatid, $casinoid, $casinofirstname, $casinolastname, $casinophone, $casinoemail, $casinobirthday, $casinodeposit, $casinodepositperiod)
{
	...
	if(empty($error))
	{
		...
		$lotteryname=preg_replace('/.*shortlottery/', '', $chatid);
		$lotteryname=$this->clean($lotteryname);
		
		$telegramchatid=preg_replace('/shortlottery.*/', '', $chatid);
		$telegramchatid=$this->clean($telegramchatid);
				
		$shortlottery='shortlottery'.$lotteryname.'';
		$shortuserslottery='shortuserslottery'.$lotteryname.'';
		$shortuserservice='shortuserservice'.$telegramchatid.'';
		$tabtempuser='tabtempuser'.$telegramchatid.'';
		$tabtempusererr='tabtempusererr'.$telegramchatid.'';
		$tabbanned='tabbanned';
		...
		if($this->cmd_sql_searchchatidtable($telegramchatid, $tabcasinousers)==FALSE)	
		{
			$lasttime=date('d.m.Y G:i');
			
			mysqli_query($con_sql2, 'INSERT INTO '.$tabcasinousers.' (chatid,casinoid,firstname,lastname,phone,email,deposit,lasttime,depositquan,birthday) VALUES ("'.$telegramchatid.'","'.$casinoid.'","'.$casinofirstname.'","'.$casinolastname.'","'.$casinophone.'","'.$casinoemail.'","'.$casinodepositperiod.'","'.$lasttime.'","'.$casinodeposit.'","'.$casinobirthday.'")');
			usleep(250000);
		...
		{
			$query=mysqli_query($con_sql2, 'SELECT * FROM '.$tabcasinousers.' WHERE chatid="'.$telegramchatid.'"');
			usleep(150000);
			$con=mysqli_num_rows($query);
			...
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET firstname = REPLACE(firstname, "'.$row[0][3].'", "'.$casinofirstname.'") WHERE chatid="'.$telegramchatid.'"');
			usleep(100000);
			
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET lastname = REPLACE(lastname, "'.$row[0][4].'", "'.$casinolastname.'") WHERE chatid="'.$telegramchatid.'"');
			usleep(100000);
			
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET phone = REPLACE(phone, "'.$row[0][5].'", "'.$casinophone.'") WHERE chatid="'.$telegramchatid.'"');
			usleep(100000);
			
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET email = REPLACE(email, "'.$row[0][6].'", "'.$casinoemail.'") WHERE chatid="'.$telegramchatid.'"');
			usleep(100000);
			
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET deposit = REPLACE(deposit, "'.$row[0][7].'", "'.$casinodepositperiod.'") WHERE chatid="'.$telegramchatid.'"');
			usleep(100000);
			
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET depositquan = REPLACE(depositquan, "'.$row[0][9].'", "'.$casinodeposit.'") WHERE chatid="'.$telegramchatid.'"');
			usleep(100000);
			
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET birthday = REPLACE(birthday, "'.$row[0][10].'", "'.$casinobirthday.'") WHERE chatid="'.$telegramchatid.'"');
			usleep(100000);
			
			mysqli_query($con_sql2, 'UPDATE '.$tabcasinousers.' SET lasttime = REPLACE(lasttime, "'.$row[0][8].'", "'.date('d.m.Y G:i').'") WHERE chatid="'.$telegramchatid.'"');
			usleep(200000);
		}
		...
		$this->cmd_start_shortlottery($lotteryname, $telegramchatid);
	}
	else
	{
		$counterlimit=DataBot::COUNTERLIMIT;
		...
		else
		{
			...
			mysqli_query($con_sql2, 'INSERT INTO '.$tabbanned.' (chatid,firstname,lastname,username,actiondate) SELECT chatid,firstname,lastname,username,actiondate FROM '.$tabusers.' WHERE chatid="'.$telegramchatid.'"');
			
			mysqli_query($con_sql2, 'DELETE FROM '.$tabusers.' WHERE chatid="'.$telegramchatid.'"');
			usleep(250000);
			
			mysqli_query($con_sql2, 'DELETE FROM '.$tabcasinousers.' WHERE chatid="'.$telegramchatid.'"');
			usleep(250000);
			
			mysqli_query($con_sql2, 'DELETE FROM '.$shortuserslottery.' WHERE chatid="'.$telegramchatid.'"');
			usleep(250000);
...
/////
	public function callback_lottery_delete_short()
	{
		...
		$text='<b>Выберите розыгрыш, который хотите удалить из канала</b>👇';
		...
		{
			$text='<b>В данный момент нет действующих розыгрышей в канале!</b>';
...
/////
	public function cmd_lottery_short_choosedelete()
	{
		...
		if(!empty(mysqli_num_rows(mysqli_query($con_sql2, 'SHOW TABLES LIKE "shortlottery%"'))))
		{
			$query=mysqli_query($con_sql2, 'SHOW TABLES LIKE "shortlottery%"');
			usleep(250000);
			$con=mysqli_num_rows($query);
			...
			for($i1=0;$i1<$ceil;$i1++)
			{
				...
				{
					$query2=mysqli_query($con_sql2, 'SELECT startdate, lotteryname FROM '.$row[$u][0].'');
					usleep(250000);
					...
					$buttext='№'.$lotteryname.' от '.$startdate.'';
...
/////
	...
	{	
		...
		if(file_exists(''.dirname(__FILE__).'/pid'.$lotteryname.'.txt'))
		{
			$pid=file_get_contents(''.dirname(__FILE__).'/pid'.$lotteryname.'.txt');
			shell_exec('kill -9 '.$pid.'');
			sleep(1);
			unlink(''.dirname(__FILE__).'/pid'.$lotteryname.'.txt');
			usleep(250000);
		}
			
		if(file_exists(''.dirname(__FILE__).'/shortlottery'.$lotteryname.'.php'))
		{
			unlink(''.dirname(__FILE__).'/shortlottery'.$lotteryname.'.php');
			usleep(250000);
		}
		...
		if(!empty(mysqli_num_rows(mysqli_query($con_sql2, 'SHOW TABLES LIKE "shortuserservice%"'))))
		{
			$query=mysqli_query($con_sql2, 'SHOW TABLES LIKE "shortuserservice%"');
			usleep(250000);
			$con=mysqli_num_rows($query);
		...
		$text='<b>Вы успешно удалили Розыгрыш №'.$lotteryname.' от '.$startdate.' из канала!</b>';
...
/////
	...
	{	
		...
		$tabchannels='tabchannels';
		$tabgroups='tabgroups';
		$historyshortlottery='historyshortlottery';
		...
		if($this->cmd_if_exists($historyshortlottery)==FALSE)
		{
			...
			if(!empty(mysqli_num_rows(mysqli_query($con_sql2, 'SHOW TABLES LIKE "shortlottery%"'))))
			{
				$query1=mysqli_query($con_sql2, 'SHOW TABLES LIKE "shortlottery%"');
				usleep(100000);
...
/////
	public function cmd_lottery_short_text()
	{
		...
		else
		{
			...
			mysqli_query($con_sql2, 'UPDATE '.$serviceshortlottery.' SET lotterytext = REPLACE(lotterytext, "'.$row[0][0].'", "'.html_entity_decode($textmessage, ENT_SUBSTITUTE).'")');
			usleep(250000);
...
/////
	public function cmd_lottery_short_image()
	{
		if(!empty($this->api->body['message']['photo']))
		{
			...
			$filedir1=str_replace(''.dirname(__FILE__).'', 'https://domain.com/telegrambot', $filesdir2);
...
/////
	public function cmd_lottery_short_textbutton()
	{
		...
		{	
			...
			mysqli_query($con_sql2, 'UPDATE '.$serviceshortlottery.' SET buttontext = REPLACE(buttontext, "'.$row[0][0].'", "'.html_entity_decode($textmessage, ENT_SUBSTITUTE).'")');
			usleep(250000);
...
/////
	public function cmd_lottery_short_message()
	{
		...
		{	
			...
			mysqli_query($con_sql2, 'UPDATE '.$serviceshortlottery.' SET usermessage = REPLACE(usermessage, "'.$row[0][0].'", "'.html_entity_decode($textmessage, ENT_SUBSTITUTE).'")');
			usleep(250000);
...
/////
	public function callback_lottery_short_cancel()
	{
		...
		if(!empty($row[0][0]))
		{
			$photopath=str_replace('https://domain.com/telegrambot', ''.dirname(__FILE__).'', $row[0][0]);
			unlink($photopath);
		}
	...
	public function callback_lottery_short_send()
	{
		...
		if(!empty($row[0][1]) && !empty($row[0][2]) && !empty($row[0][3]) && !empty($row[0][4]) && !empty($row[0][6]) && !empty($row[0][7]) && !empty($row[0][9]) && !empty($row[0][10]))
		{
			...
			elseif($future=="+")
			{
				...
				$text='Так будет выглядеть розыгрыш в канале👆👆👆'.PHP_EOL.''.PHP_EOL.'<b>Запустить розыгрыш в канал?</b>';
...
/////
	public function callback_short_lottery_send_confirm_no()
	{
		...
		$text='<b>Вы успешно отменили отправку розыгрыша в канал!</b>';
...
/////
	public function callback_short_lottery_send_confirm_yes()
	{
		...
		file_put_contents(&#039;&#039;.dirname(__FILE__).&#039;/pid'.$lotteryname.'.txt&#039;, $pid);
		...
		if($bot->cmd_if_empty(&#039;'.$shortuserslottery.'&#039;))
		{
			...
			$text=&#039;<b>Розыгрыш №'.$lotteryname.' от '.$startdate.' отменен!</b> К сожалению, никто не зарегистрировался.&#039;;
			...
			$bot->cmd_delete_shortlottery('.$lotteryname.');
		}
		else
		{
			...
			$text=&#039;<b>Розыгрыш №'.$lotteryname.' от '.$startdate.' успешно завершен!</b> Начат подсчет победителей.&#039;;
			...
			for($y=0;$y<$count;$y++)
			{
				...
				for($i=0;$i<$con;$i++)
				{
					...
					$textresults=$textresults . &#039;&#039;.$text.&#039;&#32;&#32;&#32;&#32;&#32;&#32;👉👉&#32;&#32;&#32;&#32;&#039;.$textadd.&#039;&#039;.PHP_EOL.&#039;&#039;.PHP_EOL.&#039;&#039;;
					$textresults1=$textresults1 . &#039;&#039;.$text.&#039;&#039;.PHP_EOL.&#039;&#039;;
					$textfinish=$textfinish . &#039;&#039;.$text.&#039;&#32;&#32;&#32;&#32;&#32;&#32;👉👉&#32;&#32;&#32;&#32;&#039;.$textadd.&#039;&#039;.PHP_EOL.&#039;&#039;.PHP_EOL.&#039;&#039;;
			...
			$textadmin=&#039;Сверху Вы можете увидеть список победителей розыгрыша №'.$lotteryname.' от '.$startdate.'👆👆👆&#039;.PHP_EOL.&#039;&#039;.PHP_EOL.&#039;&#039;.PHP_EOL.&#039;<b>Вас устраивают такие результаты?</b>&#039;;
	
			$bot->api->sendMessage([
			&#039;chat_id&#039; => &#039;'.$chatidadm.'&#039;,
			&#039;text&#039; => $textadmin,
			&#039;reply_markup&#039; => json_encode( [&#039;inline_keyboard&#039;=>$merge]),
			//&#039;reply_markup&#039; => json_encode( [&#039;inline_keyboard&#039;=>$bot->keyboards[&#039;inline_short_confirm_finish&#039;]]),
			//&#039;disable_notification&#039; => TRUE,
			//&#039;disable_web_page_preview&#039; => TRUE,
			&#039;parse_mode&#039;=> "HTML"
			]);
		...
		file_put_contents(''.dirname(__FILE__).'/shortlottery'.$lotteryname.'.php', html_entity_decode($insert, ENT_QUOTES) . PHP_EOL);
		sleep(1);
		shell_exec('php '.dirname(__FILE__).'/shortlottery'.$lotteryname.'.php > /dev/null 2>/dev/null &');
		
		
		$text='<b>Вы успешно запустили</b> Розыгрыш №'.$lotteryname.' в канале '.$channeltext.'!';
		...
		if($this->if_mainchannel_short($lotteryname))
		{
			if($this->cmd_if_empty($tabusers)==FALSE)
			{
				...
				$text='В канале '.$channeltext.' начался новый розыгрыш!'.PHP_EOL.''.PHP_EOL.'Чтобы принять участие, нажмите кнопку 🤝'.$buttontext.' снизу👇👇';
				$textall=$text . ''.PHP_EOL.''.PHP_EOL.''.PHP_EOL.''.PHP_EOL.''.$textall.'';
				
				$merge=[];
				$merge = [  [['text' => '🤝'.$buttontext.'', 'url' => $urllottery ]] ];
		...
		else
		{
			...
			else
			{
				...
				$chatiduser=$this->clean($chatiduser);
...
/////
	public function callback_short_results_yes($lotteryname)
	{
		if(isset($this->api->body['callback_query']["id"]))
		{
			$callback_query_id=$this->api->body['callback_query']["id"];
		...
		else
		{
			...
			$textfinish='<b>Розыгрыш №'.$lotteryname.' от '.$startdate.' успешно завершен!</b> Результаты будут размещены в канале!'.PHP_EOL.''.PHP_EOL.'👇Победители розыгрыша!!👇'.PHP_EOL.''.PHP_EOL.''.PHP_EOL.''.$usermessage.'';
			...
			/* $allwinnerschatid=explode(',', $buttontext);
			
			foreach($allwinnerschatid as $winnerschatid)
			{
				$query=mysqli_query($con_sql2, 'SELECT firstname, lastname, username FROM '.$tabusers.' WHERE chatid="'.$winnerschatid.'"');
				usleep(250000);
				$con=mysqli_num_rows($query);
			...
			$textall='<b>Розыгрыш завершен!</b>'.PHP_EOL.''.PHP_EOL.''.PHP_EOL.'';
			
			$textall=$textall . ''.$lotterytext.''.PHP_EOL.''.PHP_EOL.'';
			
			$textall=$textall . '<b>Победители розыгрыша!!</b>👇'.PHP_EOL.''.PHP_EOL.''.PHP_EOL.''.$usermessage.'';
			...
			if(preg_match('/,/', $buttontext))
			{
				$alldata=explode(',', $buttontext);
			}
			else
			{
				$alldata=[];
			...
			$textuserswinners='Розыгрыш от '.$startdate.' завершен!'.PHP_EOL.''.PHP_EOL.'<b>Поздравляю! Вы стали победителем!</b>';
		...
		$this->cmd_delete_shortlottery($lotteryname);
		
	}


/////
	public function callback_short_results_no($lotteryname)
	{
		if(isset($this->api->body['callback_query']["id"]))
		{
			$callback_query_id=$this->api->body['callback_query']["id"];
		...
		$merge=[];
		$merge[]=[['text' => 'Прислать результаты вручную', 'callback_data' => 'short_manual_results('.$lotteryname.')']]; 
		$merge[]=[['text' => 'Сгенерировать новые результаты', 'callback_data' => 'short_generate('.$lotteryname.')']];
...
/////
	public function callback_short_manual_results($lotteryname)
	{
		...
		if($this->cmd_if_exists($tabserviceadmin)==false)
		{
			mysqli_query($con_sql2, 'CREATE TABLE IF NOT EXISTS '.$tabserviceadmin.' (id INT(5) UNSIGNED AUTO_INCREMENT PRIMARY KEY,info VARCHAR(35)) ENGINE = InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;');
			usleep(250000);
			mysqli_query($con_sql2, 'INSERT INTO '.$tabserviceadmin.' (info) VALUES ("shortmanual'.$lotteryname.'")');
			usleep(250000);
		}
		else
		{
...
/////
	public function cmd_delete_shortlottery($lotteryname)
	{
		if(file_exists(''.dirname(__FILE__).'/pid'.$lotteryname.'.txt'))
		{
			unlink(''.dirname(__FILE__).'/pid'.$lotteryname.'.txt');
...
/////
	public function cmd_start_lottery()
	{
		...
		if($this->cmd_isadmin())
		{
			...
			if($chatid==$this->chatidadmin)
			{
				$this->api->sendMessage([
				'text' => $text,
				'reply_markup' => json_encode($this->keyboards['default_admin1']),
				//'disable_notification' => TRUE,
				//'disable_web_page_preview' => TRUE,
				'parse_mode'=> "HTML"
				]);
				...
				if($this->id_mainchannel())
				{
					if(file_exists(''.dirname(__FILE__).'/enddate.txt'))
					{
						...
						elseif($this->cmd_sql_searchchatidtable($chatid, $tabcasinousers))
						{
							...
							{
								$text='Чтобы <b>принять участие в розыгрыше</b>, нажмите кнопку <b>Зарегистрироваться</b>👇';
	...
	public function cmd_my_tickets()
	{
		...
		if($this->id_mainchannel())
		{
			...
			elseif($this->cmd_if_exists($tablottery))
			{
				...
				elseif($this->cmd_if_empty($tabticketsall)==FALSE && !empty(mysqli_num_rows(mysqli_query($con_sql2, 'SELECT tickets FROM '.$tabticketsall.' WHERE chatid="'.$chatid.'"'))))
				{
	...
	public function cmd_change_lottery()
	{
		...
		if($this->cmd_isadmin())
		{
			...
			if(mysqli_query($con_sql2, 'SELECT 1 FROM '.$tablottery.' LIMIT 1')==FALSE )
			{
				...
				{
					mysqli_query($con_sql2, 'CREATE TABLE '.$tabadminchange.' (id INT(5) UNSIGNED AUTO_INCREMENT PRIMARY KEY,info VARCHAR(15) DEFAULT "0") ENGINE = InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;');
					usleep(250000);
...
public function cmd_finish_data_tickets($error, $chatid, $casinoid, $casinofirstname, $casinolastname, $casinophone, $casinoemail, $casinobirthday, $casinodeposit, $casinodepositperiod)
{
	...
	if(empty($error))
	{
		...
		//if($casdeppernew==$mindep || $casdeppernew>$mindep || preg_match('/5181180343/', $chatid))
		{
			...
			if($this->cmd_if_empty($tabticketsall)==FALSE && !empty(mysqli_num_rows(mysqli_query($con_sql2, 'SELECT tickets FROM '.$tabticketsall.' WHERE chatid="'.$chatid.'"'))))
			{
				$query4=mysqli_query($con_sql2, 'SELECT tickets FROM '.$tabticketsall.' WHERE chatid="'.$chatid.'"');
				usleep(250000);
				$con4=mysqli_num_rows($query4);
			...
			if($this->cmd_if_empty($tabcasinousers)==FALSE && !empty(mysqli_num_rows(mysqli_query($con_sql2, 'SELECT promocode FROM '.$tabcasinousers.' WHERE chatid="'.$chatid.'"'))))
			{
			...
			if($this->cmd_if_empty($tabticketsall)==FALSE && !empty(mysqli_num_rows(mysqli_query($con_sql2, 'SELECT tickets FROM '.$tabticketsall.' WHERE chatid="'.$chatid.'" AND status="deposit"'))))
			{
				$query4=mysqli_query($con_sql2, 'SELECT tickets FROM '.$tabticketsall.' WHERE chatid="'.$chatid.'" AND status="deposit"');
				usleep(250000);
			...
			if($this->cmd_if_empty($tabticketsall)==FALSE && !empty(mysqli_num_rows(mysqli_query($con_sql2, 'SELECT tickets FROM '.$tabticketsall.' WHERE chatid="'.$chatid.'" AND status="refer"'))))
			{
				$query4=mysqli_query($con_sql2, 'SELECT tickets FROM '.$tabticketsall.' WHERE chatid="'.$chatid.'" AND status="refer"');
				usleep(250000);
		...
		if(!empty($this->result["message"]))
		{
			if($this->cmd_isadmin())
			{
				...
				{	
					...
					{
						$lotteryname=str_replace('shortmanual', '', $info);
						$this->cmd_short_manual_results($lotteryname);
					}
					/* elseif($info=='format')
					{
						$this->cmd_result();
			...
			else
			{
				...
				{
					$chatid=$this->result["message"]["chat"]["id"];
					$tabserviceuser='tabserviceuser'.$chatid.'';
					$shortuserservice='shortuserservice'.$chatid.'';
				}
				...
				elseif(!empty($text) && preg_match('/\/start shortlottery.*/', $text))
				{
					$lotteryname=preg_replace('/\/start shortlottery/', '', $text);
					$lotteryname=$this->clean($lotteryname);
					
					$this->cmd_start_shortlottery($lotteryname, $chatid);
				}
				elseif(!empty($tabserviceuser) && $this->cmd_if_exists($tabserviceuser))
				{
					$this->cmd_id_casino_check();
				}
				elseif(!empty($shortuserservice) && $this->cmd_if_empty($shortuserservice)==FALSE)
				{
					$con_sql2=$this->cmd_sql();
					...
					$this->cmd_id_casino_short_check($lotteryname, $chatid);
				}
			}
		}
	}
