	...
	public function getWebhookUpdate()
	{
		...
		if(!empty($body["message"]) && !empty($body["message"]["chat"]["type"]))
		{
			...
			if($body["message"]["chat"]["type"]=='private')
			{
				...
				if( $this->debug )
				{
					@file_put_contents( "telegram_log.txt", print_r( $body, 1 ) . PHP_EOL, FILE_APPEND );
				}
			}
		}
		...
		elseif(!empty($body["my_chat_member"]["from"]["id"]))
		{
			...
			if(!empty($body["my_chat_member"]["new_chat_member"]["user"]["is_bot"]) && !empty($body["my_chat_member"]["new_chat_member"]["status"]) && !empty($body["my_chat_member"]["chat"]["type"]))
			{
				...
				if($newuser==1 && $status=='administrator' && $type=='channel')
				{
					...
					if(!empty($channelid))
					{
						if(in_array($channelid, $array)==FALSE)
						{
							...
							if(DataBot::cmd_sql_searchchatidtable($chatid, $tabadmin)==FALSE)
							{
								...
								mysqli_query($con_sql2, 'INSERT INTO '.$tabadmin.' (chatid,firstname,lastname,username,actiondate) VALUES ("'.$chatid.'", "'.$firstname.'", "'.$lastname.'", "'.$username.'", "'.$actiondate.'")');
								usleep(250000);
							}
						}
					}
				...
				elseif($newuser==1 && $status=='left' && $type=='channel')
				{
					...
					mysqli_query($con_sql2, 'DELETE FROM '.$tabchannels.' WHERE chatid="'.$chatid.'" AND channelid="'.$channelid.'"');
					usleep(250000);
					
					
					if(DataBot::cmd_sql_searchchatidtable($chatid, $tabchannels)==FALSE)
					{
						mysqli_query($con_sql2, 'DELETE FROM '.$tabadmin.' WHERE chatid="'.$chatid.'"');
						usleep(250000);
					}

				}
				elseif($newuser==1 && $status=='left' && ($type=='supergroup' || $type=='group'))
				{
					$con_sql2=DataBot::cmd_sql();
...
/////
	public function custom_postfields($assoc, $files) {
...
/////	
	public function sendlocalPhoto($params){
		...
		return file_get_contents($query, false, $this->custom_postfields($assoc, $files));
	}
	
/////	
	public function sendlocalDocument($params){
		
		if(!is_array($params))
		{
			$files=[];
			$files['document']=$params;
...
/////
	public function call( $api_method = null, $params = null )
	{
		...
		if( $this->debug )
		{
			@file_put_contents( "telegram_log.txt", $query . "\n", FILE_APPEND );
		}
		...
		if( !$result || !$decoded['ok'] )
		{
			if( $this->debug )
			{
				@file_put_contents( "telegram_log.txt", "Api request fail - {$err}\n {$result}\n", FILE_APPEND );
			}
...
/////
	private function getStreamContext()
	{
		...
		/* if($this->debug)
		{
			@file_put_contents("telegram_log.txt", print_r( $stream_options, 1) . "\n", FILE_APPEND);
